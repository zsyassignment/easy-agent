// Pinned Unicode 17.0 East_Asian_Width = W (Wide) or F (Fullwidth) ranges
// (flat sorted [start,end,...]).
//
// Baked in from the official EastAsianWidth.txt — not the running Node — for the same
// reason as the emoji set: xterm.js @xterm/addon-unicode11 only knows Unicode 11 EAW,
// so it under-counts code points that later Unicode versions widened (e.g. the trigram
// block U+2630..U+2637, Wide since Unicode 16). A modern local/SSH terminal paints those
// two cells wide; if the picker scores them as one, an all-☰ title overflows the row,
// wraps, and pushes the pinned title off screen. Unioning these into the width-2 set
// keeps the table a conservative cross-terminal upper bound.
//
// Source: official https://www.unicode.org/Public/17.0.0/ucd/EastAsianWidth.txt
// (Unicode 17.0; data file dated 2025-07-24), rows with property W or F. Refresh for a
// future Unicode version by re-parsing that file and re-running generate-terminal-width.mjs.
export const EAST_ASIAN_WIDE_RANGES = [
